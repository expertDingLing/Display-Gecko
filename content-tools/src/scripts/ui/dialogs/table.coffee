class ContentTools.TableDialog extends ContentTools.DialogUI
    ...
    mount: () ->
        ...
        title = ''
        cfg = {columns: 3, head: true, title: title, split: 2}
        if @table
            if @table.firstSection().children[0].children.length == 1 and 
                @table.firstSection().children[0].children[0]._domElement.className.includes('table-title')
                    title = @table.firstSection().children[0].children[0]._domElement.innerText
            cfg = {
        ...
        @_domTitleInput = document.createElement('input')
        @_domTitleInput.setAttribute('class', 'ct-section__input')
        @_domTitleInput.setAttribute('name', 'tabletitle')
        @_domTitleInput.setAttribute('type', 'text')
        @_domTitleInput.setAttribute('value', cfg.title)
        @_domBodySection.appendChild(@_domTitleInput)
    ...
    _addDOMEventListeners: () ->
        ...
        # Add support for the head switches
        toggleSection = (ev) ->
            ev.preventDefault()
