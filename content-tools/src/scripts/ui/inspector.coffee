...
class ContentTools.TagUI extends ContentTools.AnchoredComponentUI
    ...
    _getTagName: (tagName) -> 
        switch tagName
            ...
            when 'thead' then 'Table Header'
            when 'tbody' then 'Table Body'
            when 'tr' then 'Table Row'
            when 'th' then 'Table Head'
            when 'td' then 'Table Cell'
            when 'ul' then 'Bullet List'
            when 'ol' then 'Ordered List'
            when 'li' then 'Item'
            when 'img' then 'Image'
            when 'hr' then 'Horizontal Line'
            when 'h1', 'h2', 'h3', 'p', 'pre' then 'Text'
            else tagName
    ...
    mount: (domParent, before=null) ->
        ...
        @_domElement.textContent = @_getTagName(@element.tagName())
