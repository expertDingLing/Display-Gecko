...
class ContentTools.Tools.Hr extends ContentTools.Tool

    # Convert the current text block to a paragraph (e.g <hr/>)
...
class ContentTools.Tools.Table extends ContentTools.Tool
    ...
    @apply: (element, selection, callback) ->
        ...
        dialog.addEventListener 'save', (ev) =>
            ...
            # Array.from({length: tableCfg.split})
            # Array.from({length: tableCfg.split}).forEach () =>
    ...
    @_createTable: (tableCfg) ->
        ...
        # Title
        if tableCfg.title
            title = @_createTableSection('thead', 'td', 1, tableCfg.title, {
                colspan: tableCfg.columns, 
                style: 'border: 0; font-size: 30px;',
                class: 'table-title'
                })
            head = @_createTableRow('th', tableCfg.columns)
            title.attach(head)
            table.attach(title)
    ...
    @_createTableRow: (cellTag, columns, text='', attributes={}) ->
    ...
    @_createTableSection: (sectionTag, cellTag, columns, text='', attributes={}) ->
        # Create a new table section element
        section = new ContentEdit.TableSection(sectionTag)
        row = @_createTableRow(cellTag, columns, text, attributes)
        section.attach(row)
...
class ContentTools.Tools.Image extends ContentTools.Tool
    ...
    @apply: (element, selection, callback) ->
        ...
        dialog.addEventListener 'save', (ev) =>
            ...
            # Set the image size fit to the template
            ...
            #Change the image url according to the size
