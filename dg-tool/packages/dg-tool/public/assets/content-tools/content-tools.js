...
(function() {
  ...
  _Root = (function(_super) {
    ...
    _Root.prototype.startResizing = function(element, corner, x, y, fixed) {
      ...
      var scale = 1;
      if (document.getElementById("panzoom")) {
        scale = document
        .getElementById("panzoom")
        .children[0].style.transform.substring(7)
        .split(",")[0];
      }
      this._resizingParentWidth = this._resizingParentWidth / scale;
  ...
  ContentEdit.Text = (function(_super) {
    ...
    Text.prototype.blur = function() {
      ...
      if (this.content.isWhitespace() && this.can('remove')) {
        if (this.parent() && this._tagName != 'hr') {
    ...
    Text.prototype._keyReturn = function(ev) {
      ...
      ev.preventDefault();
      if (this.content.isWhitespace() && !ev.shiftKey ^ ContentEdit.PREFER_LINE_BREAKS && this._tagName != 'hr') {
        return;
      }
      selection = ContentSelect.Range.query(this._domElement);
  ...
  ContentEdit.TagNames.get().register(ContentEdit.Text, 'address', 'blockquote', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'hr');
  ...
  ContentEdit.Image = (function(_super) {
    ...
    Image.prototype.html = function(indent) {
      ...
      if (this._attributes['top']) {
        style += "top:" + this._attributes['top'] + "px;";
      } else {
        style += "top: 0px;";        
      }
      if (this._attributes['left']) {
        style += "left:" + this._attributes['left'] + "px;";
      } else {
        style += "left: 0px;";        
      }
      img = "" + indent + "<img" + (this._attributesToString()) + style + "\"" + ">";
      if (this.a) {
        le = ContentEdit.LINE_ENDINGS;
  ...
  ContentEdit.ListItem = (function(_super) {
    ...
    ListItem.fromDOMElement = function(domElement) {
      var childNode, content, listDOMElement, listElement, listItem, listItemText, _i, _len, _ref, _ref1, tag;
      ...
      content = content.replace(/^\s+|\s+$/g, '').replace('<div>', '').replace('</div>', '')
                                                  .replace('<p>', '').replace('</p>', '').replace('<h1>', '').replace('</h1>', '')
                                                  .replace('<h2>', '').replace('</h2>', '');
      listItemText = new ContentEdit.ListItemText(content, tag);
  ...
  ContentEdit.ListItemText = (function(_super) {
    ...
    function ListItemText(content, tag) {
      if (tag == null) {
        tag = 'div';
      }
      ListItemText.__super__.constructor.call(this, tag, {}, content);
    }
    ...
    ListItemText.prototype.html = function(indent) {
      ...
      if (!this._lastCached || this._lastCached < this._modified) {
        ...
        content.optimize();
        this._lastCached = Date.now();
        this._cached = content.html().replace('<div>', '').replace('</div>', '').replace('<p>', '').replace('</p>', '')
                                    .replace('<h1>', '').replace('</h1>', '').replace('<h2>', '').replace('</h2>', '');
      }
      if (this._domElement.tagName == 'div' ){
        return "" + indent + this._cached;
      } else {
        return "" + indent + "<" + this._domElement.tagName + ">" + this._cached + "</" + this._domElement.tagName + ">";
      }
    };
  ...
  ContentEdit.TableCell = (function(_super) {
    ...
    TableCell.fromDOMElement = function(domElement) {
      var tableCell, tableCellText, tag;
      tableCell = new this(domElement.tagName, this.getDOMElementAttributes(domElement));
      if (domElement.innerHTML.indexOf('<h1>') > -1){
        tag = 'h1';
      }
      if (domElement.innerHTML.indexOf('<h2>') > -1){
        tag = 'h2';
      }
      if (domElement.innerHTML.indexOf('<p>') > -1){
        tag = 'p';
      }
      tableCellText = new ContentEdit.TableCellText(domElement.innerHTML.replace(/^\s+|\s+$/g, '').replace('<div>', '').replace('</div>', '')
                                                          .replace('<p>', '').replace('</p>', '').replace('<h1>', '').replace('</h1>', '')
                                                          .replace('<h2>', '').replace('</h2>', ''), tag);
  ...
  ContentEdit.TableCellText = (function(_super) {
    ...
    function TableCellText(content, tag) {
      if (tag == null) {
        tag = 'div';
      }
      TableCellText.__super__.constructor.call(this, tag, {}, content);
    }
    ...
    TableCellText.prototype._keyReturn = function(ev) {
      ...
      if (ev.ctrlKey && !ev.altKey){
        ...
        table.parent().attach(element, table.parent().children.indexOf(table) + 1);
        element.focus();
        return table.taint();
      }
      if (ev.altKey && !ev.ctrlKey){
        var table = this.parent().parent().parent().parent();
        element = new ContentEdit.Text('p', {}, '');
        table.parent().attach(element, table.parent().children.indexOf(table));
        element.focus();
        return table.taint();
      }
      if (!(ev.ctrlKey && ev.altKey) || (ev.ctrlKey && ev.altKey)){
        return this._keyTab({
          'shiftKey': false,
...
(function() {
  var AttributeUI, ContentTools, CropMarksUI, PropertyUI, StyleUI, exports, _EditorApp,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __slice = [].slice;
  ...
  ContentTools = {
    ...
    DEFAULT_TOOLS: [['bold', 'italic', 'horizontal-line', 'align-left', 'align-center', 'align-right'], ['heading', 'subheading', 'paragraph', 'unordered-list', 'ordered-list', 'table', 'indent', 'unindent', 'line-break'], ['image', 'video'], ['undo', 'redo', 'remove']],
    ...
    getEmbedVideoURL: function(url) {
      ...
      switch (domains[netloc]) {
        case 'youtube':
          ...
          url = "https://www.youtube.com/embed/" + id + "?rel=0;&autoplay=1&mute=1";
          paramStr = ((function() {
            var _results;
            _results = [];
            for (k in params) {
        ...
        case 'vimeo':
          ...
          url = "https://player.vimeo.com/video/" + m[2] + "?autoplay=1";
          paramStr = ((function() {
            var _results;
            _results = [];
            for (k in params) {
    ...
    getRestrictedStyleProperties: function(tagName) {
      var TextTags, restricted;
      restricted = [];
      TextTags = ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'thead', 'tr', 'td', 'th'];
      if (TextTags.includes(tagName)) {
        restricted.push('font-size');
      }
      if (tagName === 'table') {
        restricted = restricted.concat(['border-left', 'border-right', 'border-top', 'border-bottom']);
      }
      if (tagName === 'thead') {
        restricted = restricted.concat(['header-visible']);
      }
      if (tagName === 'img') {
        restricted = restricted.concat([]);
      }
      if (tagName === 'hr') {
        restricted = restricted.concat(['height']);
      }
      return restricted;
    },
  ...
  ContentTools.IgnitionUI = (function(_super) {
    ...
    IgnitionUI.prototype.mount = function() {
      ...
      this._domElement.setAttribute('id', 'ct-ignition');
      this.parent().domElement().appendChild(this._domElement);
      this._domEdit = this.constructor.createDiv(['ct-ignition__button', 'ct-ignition__button--edit']);
      this._domEdit.setAttribute('data-tut', 'reactour_screen_edit_button');
      ...
      this._domConfirm.setAttribute('data-tut', 'reactour_screen_confirm_button');
      ...
      this._domCancel.setAttribute('data-tut', 'reactour_screen_cancel_button');
    ...
    IgnitionUI.prototype._addDOMEventListeners = function() {
      this._domEdit.addEventListener('click', (function(_this) {
        return function(ev) {
          ...
          _this.edit();
          _this.show();
          document.getElementById("ct-ignition").classList.remove('ct-ignition--ready');
          return document.getElementById("ct-ignition").classList.add('ct-ignition--editing');
        };
      })(this));
      this._domConfirm.addEventListener('click', (function(_this) {
        return function(ev) {
          ev.preventDefault();
          _this.confirm();
          document.getElementById("ct-ignition").classList.add('ct-ignition--ready');
          return document.getElementById("ct-ignition").classList.remove('ct-ignition--editing');
        };
      })(this));
  ...
  ContentTools.TagUI = (function(_super) {
    ...
    TagUI.prototype._getTagName = function(tagName) {
      switch (tagName) {
        case 'table':
          return 'Table';
        case 'thead':
    ...
    TagUI.prototype.mount = function(domParent, before) {
      ...
      this._domElement.textContent = this._getTagName(this.element.tagName());
  ...
  ContentTools.ToolboxUI = (function(_super) {
    ...
    ToolboxUI.prototype.mount = function() {
      ...
      this._domElement.setAttribute('data-tut', 'reactour_screen_toolbox');
      this.parent().domElement().appendChild(this._domElement);
  ...
  ContentTools.PropertiesDialog = (function(_super) {
    ...
    function PropertiesDialog(element) {
      ...
      this._styleUIs = [];
      this._cssUIs = [];
      this.borderProperties = ['border-left', 'border-right', 'border-top', 'border-bottom'];
      this.hrProperties = ['height'];
    ...
    PropertiesDialog.prototype.changedCssClass = function() {
      var attributes, changedCssAttributes, cssUI, name, value, _i, _len, _ref;
      attributes = {};
    ...
    PropertiesDialog.prototype.mount = function() {
      var attributeNames, attributes, domActions, domTabs, lastTab, name, propertiesNames, restricted, style, styleUI, styles, value, _i, _j, _k, _len, _len1, _len2, _ref;
      ...
      this._domClass = this.constructor.createDiv(['ct-properties-dialog__class', this.element.tagName() === 'img' ? 'ct-properties-dialog--none' : '']);
      this._domClass.setAttribute('data-ct-empty', ContentEdit._('No class available for this tag'));
      this._domView.appendChild(this._domClass);
      restricted = ContentTools.getRestrictedStyleProperties(this.element.tagName());
      ...
      if (attributes['style']) {
        ...
        styles = styles.reduce(function(r, x) {
        ...
        for (name in styles) {
          value = styles[name];
          if (restricted.includes('header-visible') && name === 'display') {
            continue;
          }
          if (restricted && restricted.indexOf(name.toLowerCase()) !== -1) {
            continue;
      ...
      for (_i = 0, _len = propertiesNames.length; _i < _len; _i++) {
        name = propertiesNames[_i];
        value = '';
        if (name === 'header-visible') {
          value = styles.display && styles.display.includes('none') ? true : false;
        } else if (styles[name]) {
          value = styles[name];
        }
        if (this.borderProperties.includes(name) && !styles[name]) {
          value = 0;
        }
        if (this.hrProperties.includes(name) && !styles[name]) {
          value = 5;
        }
        this._addCssClassUI(name, value);
      }
      ...
      attributeNames.sort();
      for (_k = 0, _len2 = attributeNames.length; _k < _len2; _k++) {
      ...
      this._domClassTab = this.constructor.createDiv(['ct-control', 'ct-control--icon', 'ct-control--class', this.element.tagName() === 'img' ? 'ct-control--none' : '']);
      this._domClassTab.setAttribute('data-ct-tooltip', ContentEdit._('Class'));
      domTabs.appendChild(this._domClassTab);
      ...
      if (lastTab === 'attributes') {
        ...
        if (this.element.tagName !== 'img') {
          ContentEdit.addCSSClass(this._domElement, 'ct-properties-dialog--class');
          ContentEdit.addCSSClass(this._domClassTab, 'ct-control--active');
        } else {
          ContentEdit.addCSSClass(this._domElement, 'ct-properties-dialog--attributes');
          ContentEdit.addCSSClass(this._domAttributesTab, 'ct-control--active');
        }
    ...
    PropertiesDialog.prototype.save = function() {
      var attributes, detail, innerHTML, k, styleMerge, v, _ref;
      innerHTML = null;
      if (this._supportsCoding) {
        innerHTML = this._domInnerHTML.value;
      }
      styleMerge = '';
      _ref = this.changedCssClass();
      for (k in _ref) {
        v = _ref[k];
        styleMerge += "" + k + ":" + v + ";";
      }
      attributes = this.changedAttributes();
      if ((styleMerge != null) && styleMerge !== "") {
        if (attributes == null) {
          attributes = {};
        }
        if (attributes['style']) {
          styleMerge += attributes['style'];
        }
    ...
    PropertiesDialog.prototype._addCssClassUI = function(name, value) {
      var cssUI, dialog;
      dialog = this;
      cssUI = new PropertyUI(name, value);
      this._cssUIs.push(cssUI);
      cssUI.addEventListener('blur', function(ev) {
        var index, lastCssUI, length;
        dialog._focusedAttributeUI = null;
        ContentEdit.addCSSClass(dialog._domRemoveAttribute, 'ct-control--muted');
        ...
        return lastCssUI = dialog._cssUIs[length - 1];
      });
      cssUI.addEventListener('focus', function(ev) {
        dialog._focusedAttributeUI = this;
        return ContentEdit.removeCSSClass(dialog._domRemoveAttribute, 'ct-control--muted');
      });
      cssUI.addEventListener('namechange', function(ev) {
        var element, otherAttributeUI, restricted, valid, _i, _len, _ref;
        element = dialog.element;
        name = this.name().toLowerCase();
        restricted = ContentTools.getRestrictedAtributes(element.tagName());
    ...
    PropertiesDialog.prototype._addDOMEventListeners = function() {
      ...
      this._domClassTab.addEventListener('mousedown', (function(_this) {
        return function() {
          return selectTab('class');
        };
      })(this));
  ...
  AttributeUI = (function(_super) {
    ...
    AttributeUI.prototype.mount = function(domParent, before) {
      ...
      this._domValue.setAttribute('type', 'number');
  ...
  PropertyUI = (function(_super) {
    __extends(PropertyUI, _super);

    function PropertyUI(name, value) {
      PropertyUI.__super__.constructor.call(this);
      this._initialName = name;
      this._initialValue = value;
      this.borderProperties = ['border-left', 'border-right', 'border-top', 'border-bottom'];
      this.imgProperties = [];
    ...
    PropertyUI.prototype.getFormattedName = function(name) {
      switch (name) {
        case 'height':
          return 'Height';
        case 'font-size':
        ...
        case 'header-visible':
          return 'Header Invisible';
        case 'align-left':
          return 'Align Left';
        case 'align-right':
          return 'Align Right';
        case 'center-horizontally':
          return 'Center Horizonatlly';
    ...
    PropertyUI.prototype.name = function() {
      return this._initialName.trim();
    };

    PropertyUI.prototype.value = function() {
      if (this.name() === 'header-visible') {
        return this.applied();
      }
      if (!this._domValue.value) {
        return '';
      }
      if (this.borderProperties.includes(this._initialName.trim()) && !this._domValue.value.trim().includes('px solid')) {
        return this._domValue.value.trim() + 'px solid';
      }
      if (this.borderProperties.concat(['font-size']).includes(this._initialName.trim()) && !this._domValue.value.trim().includes('px solid')) {
        return this._domValue.value.trim() + 'px';
      }
      if (this.hrProperties.includes(this._initialName.trim()) && !this._domValue.value.trim().includes('px')) {
        return this._domValue.value.trim() + 'px';
      }
      return this._domValue.value.trim();
    };

    PropertyUI.prototype.mount = function(domParent, before) {
      if (before == null) {
        before = null;
      }
      if (this._initialName === 'header-visible' || this.imgProperties.includes(this._initialName)) {
        this._domElement = this.constructor.createDiv(['ct-attribute']);
        this._domName = document.createElement('input');
        this._domName.setAttribute('class', 'ct-attribute__name');
        this._domName.setAttribute('name', 'name');
        this._domName.setAttribute('placeholder', ContentEdit._('Name'));
        this._domName.setAttribute('type', 'text');
        this._domName.setAttribute('disabled', 'true');
        this._domName.setAttribute('value', this.getFormattedName(this._initialName));
        this._domElement.appendChild(this._domName);
        this._domValue = this.constructor.createDiv(['ct-section__switch']);
        this._domElement.appendChild(this._domValue);
        if (this._initialValue === true) {
          if (!this._domElement.classList.contains('ct-section--applied')) {
            ContentEdit.addCSSClass(this._domElement, 'ct-section--applied');
          }
        }
      } else {
      ...
      return PropertyUI.__super__.mount.call(this, domParent, before);
    };

    PropertyUI.prototype.applied = function() {
      if (this._initialName === 'header-visible') {
        return this._domElement.classList.contains('ct-section--applied');
      }
      return false;
    };

    PropertyUI.prototype.valid = function(valid) {
      if (valid) {
        return ContentEdit.removeCSSClass(this._domName, 'ct-attribute__name--invalid');
      } else {
        return ContentEdit.addCSSClass(this._domName, 'ct-attribute__name--invalid');
    ...
    PropertyUI.prototype._addDOMEventListeners = function() {
      this._domName.addEventListener('blur', (function(_this) {
        return function() {
          var name, nextDomAttribute, nextNameDom;
          name = _this.name();
      ...
      if (this._initialName === 'header-visible' || this.imgProperties.includes(this._initialName)) {
        return this._domValue.addEventListener('click', (function(_this) {
          return function() {
            if (_this._domElement.classList.contains('ct-section--applied')) {
              ContentEdit.removeCSSClass(_this._domElement, 'ct-section--applied');
            } else {
              ContentEdit.addCSSClass(_this._domElement, 'ct-section--applied');
            }
            return _this.dispatchEvent(_this.createEvent('click'));
          };
        })(this));
  ...
  ContentTools.TableDialog = (function(_super) {
    ...
    TableDialog.prototype.mount = function() {
      var cfg, domBodyLabel, domControlGroup, domHeadLabel, headCSSClasses, title;
      ...
      if (this.table) {
        if (this.table.firstSection().children[0].children.length === 1 && this.table.firstSection().children[0].children[0]._domElement.className.includes('table-title')) {
          title = this.table.firstSection().children[0].children[0]._domElement.innerText;
        }
        cfg = {
      ...
      this._domView.appendChild(this._domBodySection);
      domBodyLabel = this.constructor.createDiv(['ct-section__label']);
      domBodyLabel.textContent = ContentEdit._('Title');
      this._domBodySection.appendChild(domBodyLabel);
      this._domTitleInput = document.createElement('input');
      this._domTitleInput.setAttribute('class', 'ct-section__input');
      this._domTitleInput.setAttribute('name', 'tabletitle');
      this._domTitleInput.setAttribute('type', 'text');
      this._domTitleInput.setAttribute('value', cfg.title);
      this._domBodySection.appendChild(this._domTitleInput);
    ...
    TableDialog.prototype.unmount = function() {
      ...
      return this._domHeadSwitch = null;
    };
  ...
  ContentTools.TextDialog = (function(_super) {
    __extends(TextDialog, _super);

    function TextDialog(config, text) {
      this.config = config;
      this.text = text;
      if (this.text) {
        TextDialog.__super__.constructor.call(this, 'Update Font size to text tags');
      } else {
        TextDialog.__super__.constructor.call(this, 'Insert text tags with font size');
      }
    }

    TextDialog.prototype.mount = function() {
      var cfg, domBodyLabel, domControlGroup;
      TextDialog.__super__.mount.call(this);
      cfg = this.config;
      if (this.text) {
        cfg = {
          size: 14
      ...
      this._domBodyInput.setAttribute('class', 'ct-section__input');
      this._domBodyInput.setAttribute('maxlength', '2');
      this._domBodyInput.setAttribute('name', 'fontsize');
      this._domBodyInput.setAttribute('type', 'text');
      this._domBodyInput.setAttribute('value', cfg.size);
      this._domBodySection.appendChild(this._domBodyInput);
      domControlGroup = this.constructor.createDiv(['ct-control-group', 'ct-control-group--right']);
      this._domControls.appendChild(domControlGroup);
      this._domApply = this.constructor.createDiv(['ct-control', 'ct-control--text', 'ct-control--apply']);
    ...
    TextDialog.prototype.save = function() {
      var detail;
      detail = {
        size: parseInt(this._domBodyInput.value)
      };
      return this.dispatchEvent(this.createEvent('save', detail));
    };

    TextDialog.prototype.unmount = function() {
      TextDialog.__super__.unmount.call(this);
      this._domBodyInput = null;
      this._domBodySection = null;
      return this._domApply = null;
    };

    TextDialog.prototype._addDOMEventListeners = function() {
      TextDialog.__super__._addDOMEventListeners.call(this);
  ...
  ContentTools.HTMLCleaner = (function() {
    ...
    HTMLCleaner.DEFAULT_TAG_WHITELIST = ['a', 'address', 'b', 'blockquote', 'code', 'del', 'em', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'i', 'ins', 'li', 'ol', 'p', 'pre', 'strong', 'sup', 'table', 'tbody', 'td', 'th', 'thead', 'tr', 'u', 'ul', '#text'];
  ...
  ContentTools.StylePalette = (function() {
    ...
    StylePalette._styles = [new ContentTools.Style('Align Left', 'align-left', ['img']), new ContentTools.Style('Align Right', 'align-right', ['img']), new ContentTools.Style('Center Horizontally', 'center-horizontally', ['img']), new ContentTools.Style('Center Vertically', 'center-vertically', ['img'])];
  ...
  ContentTools.Tools.Hr = (function(_super) {
    ...
    ContentTools.ToolShelf.stow(Hr, 'horizontal-line');

    Hr.label = 'Horizontal Line';

    Hr.icon = 'horizontal-line';

    Hr.tagName = 'hr';

    Hr.canApply = function(element, selection) {
      if (element.isFixed()) {
        return false;
      }
      return element !== void 0;
    ...
    Hr.apply = function(element, selection, callback) {
      var cursor, forceAdd, hrElement, pElement, tail, tip, toolDetail;
      forceAdd = this.editor().ctrlDown();
      toolDetail = {
        'tool': this,
        'element': element,
      ...
      element.content = tip.trim();
      element.updateInnerHTML();
      hrElement = new element.constructor('hr', {}, tail.trim());
      pElement = new element.constructor('p', {}, tail.trim());
      element.parent().attach(hrElement, element.parent().children.indexOf(element) + 1);
      element.parent().attach(pElement, element.parent().children.indexOf(element) + 2);
      pElement.focus();
      element.taint();
      cursor = selection.get()[0] + 2;
      selection.set(cursor, cursor);
      pElement.selection(selection);
      callback(true);
      return this.dispatchEditorEvent('tool-applied', toolDetail);
    };
  ...
  ContentTools.Tools.Table = (function(_super) {
    ...
    Table._createTable = function(tableCfg) {
      var body, head, table, title;
      table = new ContentEdit.Table();
      if (tableCfg.title) {
        title = this._createTableSection('thead', 'td', 1, tableCfg.title, {
          colspan: tableCfg.columns,
          style: 'border: 0; font-size: 30px;',
          "class": 'table-title'
        });
        head = this._createTableRow('th', tableCfg.columns);
        title.attach(head);
        table.attach(title);
      } else if (tableCfg.head) {
    ...
    Table._createTableRow = function(cellTag, columns, text, attributes) {
      var cell, cellText, i, row, _i;
      if (text == null) {
        text = '';
      }
      if (attributes == null) {
      ...
      for (i = _i = 0; 0 <= columns ? _i < columns : _i > columns; i = 0 <= columns ? ++_i : --_i) {
        cell = new ContentEdit.TableCell(cellTag, attributes);
        row.attach(cell);
        cellText = new ContentEdit.TableCellText(text);
        cell.attach(cellText);
      }
      return row;
    };

    Table._createTableSection = function(sectionTag, cellTag, columns, text, attributes) {
      var row, section;
      if (text == null) {
        text = '';
      }
      ...
      row = this._createTableRow(cellTag, columns, text, attributes);
      section.attach(row);
      return section;
    };

    Table._updateTable = function(tableCfg, table) {
      var columns, head, section, _i, _len, _ref;
  ...
  ContentTools.Tools.Image = (function(_super) {
    ...
    Image.apply = function(element, selection, callback) {
      ...
      dialog.addEventListener('save', (function(_this) {
        return function(ev) {
          var detail, image, imageAttrs, imageSize, imageURL, index, left, node, ratio, scale, templateRect, top, _ref;
          detail = ev.detail();
          ...
          templateRect = element.domElement().closest("div div").parentElement.getBoundingClientRect();
          ratio = Math.min(templateRect.width / imageSize[0], templateRect.height / imageSize[1]);
          scale = document.getElementById("panzoom").children[0].style.transform.substring(7).split(",")[0];
          if (templateRect.width / imageSize[0] > templateRect.height / imageSize[1]) {
            top = 0.0;
            left = (templateRect.width - imageSize[0] * ratio) / 2 / scale;
          } else {
            top = (templateRect.height - imageSize[1] * ratio) / 2 / scale;
            left = 0.0;
          }
          imageSize[0] = imageSize[0] * ratio / scale;
          imageSize[1] = imageSize[1] * ratio / scale;
          imageURL = imageURL.replace("w_400", "w_" + parseInt(Math.max(imageSize[0], imageSize[1])));
          imageURL = imageURL.replace("h_400", "h_" + parseInt(Math.max(imageSize[0], imageSize[1])));
          imageAttrs.height = imageSize[1];
          imageAttrs.src = imageURL;
          imageAttrs.width = imageSize[0];
          imageAttrs.top = top;
          imageAttrs.left = left;
          if (element.type() === 'ImageFixture') {
