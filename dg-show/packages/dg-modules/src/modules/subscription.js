...
import {
  ...
  SUBSCRIBE_PREMIUM_PLAN,
  CANCEL_PREMIUM_PLAN,
  SET_TRIAL_BANNER_VISIBLE
} from "./actionTypes";
...
// Selectors
// ==================================
export const biilingSubscriptionSelector = createSelector(
  state => state.subscription,
  subscription => subscription.data
);

export const trialBannerVisibleSelector = createSelector(
  state => state.subscription,
  subscription => subscription.trialBannerVisible
);

export const isTrialSelector = createSelector(
  biilingSubscriptionSelector,
  subscription => {
    const { trialPeriod, status } = subscription || {};
...
export const getBillingSubscription = createAction(
  GET_BILLING_SUBSCRIPTION,
  () => {
...
export const subscribeToPremiumPlan = createAction(
  SUBSCRIBE_PREMIUM_PLAN,
  paymentMethodNonce => {
...
const ACTION_HANDLERS = {
  [fulfilled(getBillingSubscription)]: (state, action) => {
    const { trialPeriod, status } = action.payload;

    return {
      ...state,
      data: action.payload,
      trialBannerVisible:
        trialPeriod && status !== "Active" && status !== "Pending"
    };
  },
