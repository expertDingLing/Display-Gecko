...
export const getAllScreens = createAction(
  GET_ALL_SCREENS,
  () => (_, getState) => {
    if (window.navigator.onLine) {
...
export const reorderScreens = createAction(
  REORDER_SCREENS,
  (startIndex, endIndex) => (dispatch, getState) => {
    ...
    if (endIndex === 0) {
      ...
      removed.orderWeight =
        (screenList[endIndex - 1].orderWeight +
          screenList[endIndex + 1].orderWeight) /
        2;
    }
    ...
    return API.put(config.apiGateway.name, `/company/screens/${removed.id}`, {
      body: { orderWeight: removed.orderWeight },
    });
  }
);

export const createScreenFromTemplate = createAction(
  CREATE_SCREEN_FROM_TEMPLATE,
  (template, insertIndex) => async (dispatch, getState) => {
    const screenList = [...screenListSelector(getState())];
    ...
    if (screenList.length === 0) {
      ...
      orderWeight =
        (screenList[insertIndex - 1].orderWeight +
          screenList[insertIndex].orderWeight) /
        2;
    }
...
const ACTION_HANDLERS = {
  [fulfilled(getAllScreens)]: (state, action) => ({
    ...state,
    list: action.payload,
  }),
  [SET_SCREENS_SILENTLY]: (state, action) => ({
    ...state,
    list: action.payload,
  }),
  ...
  [fulfilled(updateScreen)]: (state, action) => ({
    ...state,
    list: state.list.map((screen) =>
      screen.id === action.payload.id ? action.payload : screen
    ),
  }),
  [fulfilled(deleteScreen)]: (state, action) => ({
    ...state,
