...
export const userSelector = createSelector(state => state.user, user => user.data); // ==================================
// Actions
// ==================================
...
export const updateUser = createAction(UPDATE_USER, user => {
  return API.put(config.apiGateway.name, "/company/users/me", {
    body: user
  });
});
...
const ACTION_HANDLERS = {
  [fulfilled(getUser)]: (state, action) => ({ ...state,
    data: action.payload
  }),
  [fulfilled(updateUser)]: (state, action) => ({ ...state,
    data: action.payload
  })
}; // ==================================
// Reducer
